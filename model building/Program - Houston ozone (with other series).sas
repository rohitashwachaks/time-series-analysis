...
RUN;
* First try: original ozone *;
* Stepwise regression *;
* Test of H with White's SPEC option on final model *;
proc reg data=ozone;
  model ozone =WINDSPEED    MAXTEMP RAIN    NOX RADIATION   OPAQUENESS 
  lag1ozone       lag2ozone       lag3ozone       lag4ozone     
  lag1WindSpeed   lag1MaxTemp     lag1Rain        lag1NOx       
  lag1Radiation   lag1Opaqueness 
  / selection=stepwise spec;
  plot r. * p.;
  plot r. * (windspeed opaqueness lag1WindSpeed lag1ozone NOx maxtemp);
RUN;
* The model selects 5 predictors (windspeed opaqueness lag1WindSpeed lag1ozone NOx maxtemp)
    with R-sq = 0.6338 and RMSE = sqrt(11.43263). *;
* SPEC accepts H *;
* Test L and I *;
proc autoreg data=ozone;
  model ozone = windspeed opaqueness lag1WindSpeed lag1ozone NOx maxtemp / reset dwprob;
run;
* RESET rejects L, DW accepts I                *;
* Plot of residuals v windspeed suggests curve *;
************************************************;

* Second try: log ozone and fix curve in residual plot *;
* Uncomment ozone and WindSpeed2 lines in DATA step *;
* Stepwise regression *;
* Test of H with White's SPEC option on selected model *;
proc reg data=ozone;
  model ozone =WINDSPEED    MAXTEMP RAIN    NOX RADIATION   OPAQUENESS 
  lag1ozone       lag2ozone       lag3ozone       lag4ozone     
  lag1WindSpeed   lag1MaxTemp     lag1Rain        lag1NOx       
  lag1Radiation   lag1Opaqueness  WindSpeed2
  / selection=stepwise spec;
  var date;
  plot r. * p.;
  plot r. * date;
RUN;
* The model selects 7 predictors (windspeed opaqueness windspeed2 lag1ozone lag1WindSpeed lag1NOx lag1Rain) 
    with R-sq = 0.7973 and RMSE = sqrt(0.05059)[log scale]. *;
* Elect to drop marginal lag1Rain *;
* SPEC accepts H *;
* Test I and L  with selected model *;
* Durbin-Watson test of I - Ramsey's RESET test of L *;
proc autoreg data=ozone;
  model ozone =WINDSPEED                                OPAQUENESS 
  lag1ozone                                                     
  lag1WindSpeed                   lag1Rain        lag1NOx       
                                  WindSpeed2  
  / reset dw=4 dwprob;
RUN;
* DW rejects I at lags 3 and 4 *;
* Tweak model by adding lags 2-4 of ozone *;
proc autoreg data=ozone;
  model ozone =WINDSPEED                                OPAQUENESS 
  lag1ozone       lag2ozone       lag3ozone       lag4ozone     
  lag1WindSpeed                   lag1Rain        lag1NOx       
  ...
  / reset dw=4 dwprob;
RUN;
* Further tweak in interest of parsimony: Remove lag1Rain and lag2ozone *;
proc autoreg data=ozone;
  model ozone =WINDSPEED                                OPAQUENESS 
  lag1ozone                       lag3ozone       lag4ozone     
  lag1WindSpeed                                   lag1NOx       
                                  WindSpeed2  
  / reset dw=4 dwprob;
RUN;
* Retest H *;
proc reg data=ozone;
  ...
  lag1WindSpeed                                   lag1NOx       
                                  WindSpeed2  
  / spec      dwprob;
  output out = ozone  p=pred_ozone r=resid_ozone;
RUN;
* H passes SPEC *;
* Four tests of N: *;
...
RUN;
* All 4 tests pass N *;
* Final model has 8 predictors: (WINDSPEED OPAQUENESS lag1ozone lag3ozone lag4ozone lag1WindSpeed lag1NOx WindSpeed2) 
    with R-sq = 0.7971 and RMSE = 0.22605 and all validity tests pass. *;

* But the final model cannot be used as-is for forecasting b/c of contemporaneous predictors windspeed and opaqueness *;
* Tweak for forecasting: lag the contemporaneous predictors *;
proc reg data=ozone;
  model ozone =                                         lag1OPAQUENESS 
  lag1ozone                       lag3ozone       lag4ozone     
  lag1WindSpeed                                   lag1NOx       
                                  lag1WindSpeed2  
  / spec      dwprob;
  output out = ozone  p=pred_ozone r=resid_ozone;
RUN;
* R-sq drops to 0.3406, RMSE increases to 0.40568, some predictors lose significance *;
* To develop a model for forecasting, may consider restarting modeling process 
    - this time using only predictors that will be known at time forecast is made. *;
